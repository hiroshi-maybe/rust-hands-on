...
impl<const NRES: usize, const NTH: usize> Resource<NRES, NTH> {
    fn new(available: [usize; NRES], max: [[usize; NRES]; NTH]) -> Self {
        Resource {
            ...
            allocation: [[0; NRES]; NTH], // current allocation for the thread `i`
            max,                          // required resource by the thread `i`
        }
    }
    ...
    fn is_safe(&self) -> bool {
        ...
        loop {
            ...
            // all the threads are available
            if num_true == NTH {
                return true;
            }

            // no additional threads are available
            if !found {
                break;
            }

            // continue if any threads are available
        }
