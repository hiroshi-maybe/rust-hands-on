        ...
        for (int i = 0; i < num_events; i++) {
            ...
            if (evList[i].ident == local_s) {
                ...
                if (conn_add(fd) == 0) {
                    ...
                    int res = kevent(kq, &evSet, 1, NULL, 0, NULL);
                    printf("registered fd %d from %d to kqueue (res_code = %d)\n", fd, local_s, res);
                    send_welcome_msg(fd);
                } else {
...
int create_socket_and_listen() {
    ...
    getaddrinfo("127.0.0.1", "10000", &hints, &addr);
...
int main(int argc, char *argv[]) {
    ...
    kevent(kq, &evSet, 1, NULL, 0, NULL);
    printf("registered fd %d for kqueue %d\n", local_s, kq);
