use std::{collections::HashMap, fs, io, net::{AddrParseError, Ipv4Addr, SocketAddr}, str::FromStr};
...
pub fn load_env() -> HashMap<String, String> {
    return fs::read_to_string(".env").expect("Failed to read .env file")
        .lines()
        .filter_map(|line| {
...
fn parse_and_insert(key: &str, env: &HashMap<String, String>, store: &mut HashMap<String, Ipv4Addr>) -> Result<(), AddrParseError> {
...
pub fn get_and_parse_addr<F>(key: &str, env: &HashMap<String, String>) -> Result<F, <F as FromStr>::Err> where F: FromStr {
    env
        .get(key)
